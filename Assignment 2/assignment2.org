...
#+BEGIN_SRC c :tangle hash.h
  ...
  void run_insertions_mut(node_t *kv_pairs, hashtable_t *table, unsigned len, unsigned thread_num) {
    ...
    for (unsigned i = 0; i < thread_num; i++) {
      ...
      ht_arg_t insertion_arg = {table, &kv_pairs[start], len / thread_num, &barrier};
      pthread_create(&threads[i], NULL, *insert_into_ht_mut, (void *)(&insertion_arg));
...
#+BEGIN_SRC c :tangle hash.h
  ...
  void run_retrievals_mut(node_t *kv_pairs, hashtable_t *table, unsigned len, unsigned thread_num) {
    ...
    for (unsigned i = 0; i < thread_num; i++) {
      unsigned start = i * len / thread_num;
      //unsigned end = (i + 1) * len / thread_num;
      ht_arg_t retrieval_arg = {table, &kv_pairs[start], len / thread_num, &barrier};
      pthread_create(&threads[i], NULL, *retrieve_from_ht_mut, (void *)(&retrieval_arg));
      //printf("Created thread %u\n", i);
    }
...
#+BEGIN_SRC c :tangle hash.h
  ...
  void run_insertions_rw(node_t *kv_pairs, hashtable_t *table, unsigned len, unsigned thread_num) {
    ...
    for (unsigned i = 0; i < thread_num; i++) {
      ...
      //unsigned end = (i + 1) * len / thread_num;
      ht_arg_t insertion_arg = {table, &kv_pairs[start], floor(len / thread_num), &barrier};
      pthread_create(&threads[i], NULL, *insert_into_ht_rw, (void *)(&insertion_arg));
      printf("Created thread %u\n", i);
    }
...
#+BEGIN_SRC c :tangle hash.h
  ...
  void run_retrievals_rw(node_t *kv_pairs, hashtable_t *table, unsigned len, unsigned thread_num) {
    ...
    for (unsigned i = 0; i < thread_num; i++) {
      unsigned start = floor(i * len / thread_num);
      //unsigned end = (i + 1) * len / thread_num;
      ht_arg_t retrieval_arg = {table, &kv_pairs[start], floor(len / thread_num), &barrier};
      pthread_create(&threads[i], NULL, *retrieve_from_ht_rw, (void *)(&retrieval_arg));
      printf("Created thread %u\n", i);
    }
...
#+BEGIN_SRC c :tangle assignment2.c
  ...
  int main(int argc, char **argv) {
    ...
    double time_puzzle, time_ht_mut_ins, time_ht_mut_ret, time_board_gen;

    const unsigned MAX_THREADS = 4;
    const unsigned MUTEX_COUNTS = 4;
    const unsigned NUM_BOARDS = 50000;
